    ...
    return (
        <div className="flex flex-row items-start gap-8">
            <div className="flex flex-col items-center gap-4">
                ...
                />
                <p
                    className={`text-2xl font-bold text-center ${isLastResult ? 'text-yellow-400 text-3xl' : 'text-white'} ${isRotating && !isLastResult ? 'animate-bounce' : ''}`}
                >
                    {prediction_number}
                </p>
            </div>
                ...
                {results.map((result, index) => (
                    <div
                        className={cn(
                            ...
                            {
                                'animate-pulse scale-animation bg-yellow-500 text-black border-yellow-500':
                                    result > 3,
